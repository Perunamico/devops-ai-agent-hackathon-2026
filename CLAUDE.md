...
- Firebase Auth をセッション永続化（`browserSessionPersistence`）で初期化
  - URL を開き直す・ブラウザを閉じて再訪 → 必ずログインから（過去の IndexedDB セッションは復元しない）
  - 同じタブ内のリロード・画面遷移ではログイン維持
  - タブごとに独立したセッションになる点に注意（別タブで開くと再ログインが必要）
