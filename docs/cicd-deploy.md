...
Cloud Run deploy は WIF を使い、Firebase Hosting deploy は `FirebaseExtended/action-hosting-deploy@v0` と `FIREBASE_SERVICE_ACCOUNT` を使います。
...
- Firebase Hosting deploy は Firebase CLI の WIF 認証互換性を避けるため、`FirebaseExtended/action-hosting-deploy@v0` と専用サービスアカウント JSON を使います。
