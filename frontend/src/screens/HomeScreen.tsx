...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    // 命名モードでは hand を切り替えず固定ループさせる（Infinity ループ）。
    const minLoops =
      phase === 'naming' && currentAnim === 'hand'
        ? Infinity
        : ANIM_CONFIG[currentAnim].minLoops;

    player.start(minLoops, () => {
      if (phase === 'naming') {
        // イントロ: shake 完了 → hand へ（hand 自身は Infinity で固定ループ）
        if (currentAnim !== 'hand') setCurrentAnim('hand');
        return;
      }
      if (currentAnim !== 'hand') lastAnimRef.current = currentAnim;
      const next = pickNextAnim(currentAnim, lastAnimRef.current);
      // 未デコードの interlude を選んだ場合は hand に戻す
      setCurrentAnim(playersRef.current[next] ? next : 'hand');
    });

    return () => player.stop();
  }, [currentAnim, coreReady, phase]);
